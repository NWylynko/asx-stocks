...
export const asxCompanyJson = (ticker: string): string =>
  `https://www.asx.com.au/asx/1/company/${ticker}?fields=primary_share,latest_annual_reports,last_dividend,primary_share.indices`;

/**
 * Endpoint to pull individual securities data
 */
export const asxSingleJson = (ticker: string): string => `https://www.asx.com.au/asx/1/share/${ticker}`;

/**
 * Endpoint to pull annoucements
 */
export const asxSnnouncementsJson = (ticker: string): string =>
...
export const floatauHistoricalCsv = (ticker: string): string =>
  `http://float.com.au/download/${ticker}.csv?format=stockeasy`;
